...
export default function SignupPage() {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      // NEW ROUTE - Account Type removed, goes to select-role
    ...
    >
      ...
      >
        <div className="text-center">
          ...
          <div className="mt-2 inline-flex px-3 py-1 rounded-full bg-[#F6F1E6] border border-black/5">
            <span className="text-[10px] font-black tracking-[0.15em] opacity-50">STEP 1 OF 3</span>
          </div>
        </div>
        ...
        <div className="mt-6">
          ...
          <input
            ...
            className={`mt-2 w-full h-[52px] px-5 rounded-[14px] border text-[15.5px] font-medium outline-none transition-all ${focused==="name"? "bg-white border-black/20 shadow-[0_0_0_4px_rgba(0,0,0,0.05)]" : "bg-[#F6F1E6] border-black/10"}`}
            style={{ color: "#111827" }}
          />
          ...
          <input
            ...
            className={`mt-2 w-full h-[52px] px-5 rounded-[14px] border text-[15.5px] font-medium outline-none transition-all ${focused==="contact"? "bg-white border-black/20 shadow-[0_0_0_4px_rgba(0,0,0,0.05)]" : "bg-[#F6F1E6] border-black/10"}`}
            style={{ color: "#111827" }}
          />
          ...
          <div className="relative mt-2">
            <input
              ...
              className={`w-full h-[52px] px-5 pr-12 rounded-[14px] border text-[15.5px] font-medium outline-none transition-all ${focused==="password"? "bg-white border-black/20 shadow-[0_0_0_4px_rgba(0,0,0,0.05)]" : "bg-[#F6F1E6] border-black/10"}`}
              style={{ color: "#111827" }}
            />
            <button
              onClick={() => setShow(!show)}
              type="button"
              className="absolute right-4 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white border border-black/10 flex items-center justify-center"
          ...
          <input
            ...
            className={`mt-2 w-full h-[52px] px-5 rounded-[14px] border text-[15.5px] font-medium outline-none transition-all ${focused==="confirm"? "bg-white border-black/20 shadow-[0_0_0_4px_rgba(0,0,0,0.05)]" : "bg-[#F6F1E6] border-black/10"}`}
            style={{ color: "#111827" }}
          />

          <label className="mt-5 flex items-start gap-2 cursor-pointer p-3 rounded-[12px] bg-[#F6F1E6] border border-black/5">
            <input
              type="checkbox"
              checked={terms}
          ...
          {err && (
            <div className="mt-4 px-4 py-2.5 rounded-[10px] bg-red-50 text-[13px] font-bold border border-red-200 text-red-600">
              {err}
            </div>
          )}

...
export default function ProfilePage() {
  ...
  // personal/user_39025 ho ya sirf user_39025 ho, dono kaam karega
  ...
  const load = async () => {
    ...
    if(error ||!p){
      console.log("Profile not found for:", username, error)
      setNotFound(true)
      return
    }
    setProfile(p)
    ...
    try {
      ...
      if(user && user.id!==p.id){
        // view count - fail bhi ho to koi farak nahi
        await supabase.from('profile_views').insert({ viewer_id: user.id, viewed_id: p.id }).then(()=>{})
      }
  ...
  if(notFound) return (
    <div className="min-h-screen bg-[#E8DCC5] flex items-center justify-center p-6">
      <div className="bg-white rounded-[32px] p-8 text-center max-w-[360px]">
        <p className="text-[48px]">😕</p>
        <p className="font-black text-[20px] mt-2">User not found</p>
        <p className="text-[13px] opacity-60 mt-1">@{username} naam ka user profiles table me nahi hai</p>
        <p className="text-[11px] bg-[#F6F1E6] rounded-full px-3 py-2 mt-4">Supabase &gt; Table Editor &gt; profiles &gt; Insert row</p>
      </div>
    </div>
  )
      ...
      <div className="max-w-[480px] mx-auto">
        ...
        <div className="px-3 space-y-3 mt-2">
          <div className="bg-white rounded-[32px] p-6 pt-20 shadow-[0_8px_32px_rgba(0,0,0,0.06)] border border-white">
            <h1 className="text-[32px] font-black tracking-tight text-center leading-none">{profile.full_name || username}</h1>
